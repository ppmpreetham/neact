...
function updateDom(dom, prevProps, nextProps) {
    ...
    Object.keys(prevProps)
        ...
        ) // filter out properties that are not in the new props or have changed
        .forEach((name) => {
...
function useState(initial) {
    const oldHook =
        ...
        wipFiber.alternate.hooks[hookIndex] // get the previous hook if it exists
    const hook = {
...
function App(props) {
    ...
    return (
        <h1 onClick={() => setState((prevState) => prevState + 1)}>
            Hello, {props.name}. You clicked {state} times.
        </h1>
    )
}
